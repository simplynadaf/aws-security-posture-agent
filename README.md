...
| 2 | **SecurityScanner** | Cloud Security Analyst | `security_group_analyzer`, `s3_config_checker`, `iam_analyzer` |
...
```bash
# Clone
git clone https://github.com/simplynadaf/security-posture-agent.git
...
- `gen_ai.execute_tool` spans for each boto3 tool execution
...
**Fix** (see `src/security_posture/tools/iam_analyzer.py`):
...
**Result**: IAM tool output reduced 42%, SecurityScanner agent 12% faster, 66% fewer API calls.
